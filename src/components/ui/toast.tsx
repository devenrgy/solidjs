	...
	return (
		<Portal>
			<ToastPrimitive.Region>
				<ToastPrimitive.List
					class={cn(
						'fixed top-0 z-[100] flex max-h-screen w-full flex-col-reverse gap-2 p-4 sm:top-auto sm:right-0 sm:bottom-0 md:max-w-[420px] sm:flex-col',
						local.class,
					)}
					{...others}
				/>
...
const ToastClose = <T extends ValidComponent = 'button'>(props: PolymorphicProps<T, ToastCloseButtonProps>) => {
	...
	return (
		<ToastPrimitive.CloseButton
			class={cn(
				'absolute top-2 right-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity group-[.destructive]:text-destructive-foreground group-[.error]:text-error-foreground group-[.success]:text-success-foreground group-[.warning]:text-warning-foreground focus:opacity-100 group-hover:opacity-100 focus:outline-none focus:ring-2',
				local.class,
			)}
			{...others}
		>
