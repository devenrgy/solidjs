	...
	return (
		<SheetPortal position={local.position}>
			...
			<SheetPrimitive.Content class={cn(sheetVariants({ position: local.position }), local.class)} {...others}>
				{local.children}
				<SheetPrimitive.CloseButton class='absolute top-4 right-4 rounded-sm opacity-70 ring-offset-background transition-opacity disabled:pointer-events-none data-[state=open]:bg-secondary hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2'>
					<svg
...
const SheetTitle = <T extends ValidComponent = 'h2'>(props: PolymorphicProps<T, DialogTitleProps>) => {
	...
	return <SheetPrimitive.Title class={cn('font-semibold text-foreground text-lg', local.class)} {...others} />;
};
