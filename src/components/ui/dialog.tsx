...
const DialogContent = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, DialogContentProps>) => {
	...
	return (
		<DialogPortal>
			...
			>
				{props.children}
				<DialogPrimitive.CloseButton class='absolute top-4 right-4 rounded-sm opacity-70 ring-offset-background transition-opacity disabled:pointer-events-none data-[expanded]:bg-accent data-[expanded]:text-muted-foreground hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2'>
					<svg
...
const DialogTitle = <T extends ValidComponent = 'h2'>(props: PolymorphicProps<T, DialogTitleProps>) => {
	...
	return (
		<DialogPrimitive.Title class={cn('font-semibold text-lg leading-none tracking-tight', props.class)} {...rest} />
	);
};
